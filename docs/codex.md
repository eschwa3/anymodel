...
The `@v1.1.1` tag pin is deliberate, not cosmetic: an unpinned
...
`codex mcp add anymodel -- uvx anymodel-subagents==1.1.1`, pinned the same
...
writing its value — keeping the `@v1.1.1` pin in `args`:
...
```bash
codex mcp add anymodel --env BRAVE_API_KEY=<your-brave-key> -- uvx --from git+https://github.com/eschwa3/anymodel@v1.1.1 anymodel-subagents
```
