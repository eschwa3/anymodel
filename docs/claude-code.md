...
(`git+https://github.com/eschwa3/anymodel@v1.1.0`) rather than a moving branch
...
The `@v1.1.0` pin matters: without it, `uvx --from git+...` resolves to
...
`uvx anymodel-subagents==1.1.0` (`"command": "uvx"`), which is faster to
