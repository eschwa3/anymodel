...
class JobRepository:
    ...
    def mark_done(self, job_id: int) -> None:
        self.conn.execute(
            ...
            (JobStatus.DONE.value, utcnow().isoformat(), job_id),
        )
        self.conn.commit()
    ...
    def mark_failed_retry(self, job_id: int, retry_count: int) -> None:
        self.conn.execute(
            ...
            (JobStatus.PENDING.value, retry_count, utcnow().isoformat(), job_id),
        )
        self.conn.commit()
    ...
    def mark_dead(self, job_id: int, retry_count: int) -> None:
        self.conn.execute(
            "UPDATE jobs SET status = ?, retry_count = ?, updated_at = ? WHERE id = ?",
            (JobStatus.DEAD.value, retry_count, utcnow().isoformat(), job_id),
        )
        self.conn.commit()
    ...
    def create(self, invoice: Invoice) -> Invoice:
        cur = self.conn.execute(
            ...
            (
                ...
                invoice.total_cents,
                invoice.currency,
                invoice.created_at or utcnow().isoformat(),
            ),
        )
