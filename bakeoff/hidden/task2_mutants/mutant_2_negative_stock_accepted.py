"""Mutant 2: drops the negative-initial_stock validation.

A negative `initial_stock` is silently treated like zero stock instead
of being reported as a row error. A good test suite for Task 2 checks
that a negative initial_stock lands in `result.errors`, so it should
FAIL against this mutant.
"""
...
def _parse_row(row: dict[str, str], line_number: int) -> Item:
    ...
    # MUTATION: negative-stock validation removed -- falls through to the
    # "<= 0 means register-only, no stock" path below instead of erroring.
