"""Mutant 3: treats an empty file as a validation error instead of an
empty-but-valid import.

A good test suite for Task 2 checks that empty input imports cleanly
(no items, no errors, no exception), so it should FAIL against this
mutant, which raises ImportValidationError instead.
"""
...
def import_catalog(source: str | Path | StringIO) -> ImportResult:
    ...
    # MUTATION: empty input now raises instead of returning an empty,
    # successful ImportResult.
    if not text.strip():
        raise ImportValidationError("catalog file is empty")
