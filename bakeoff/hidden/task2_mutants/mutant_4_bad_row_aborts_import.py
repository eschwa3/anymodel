"""Mutant 4: a single malformed row aborts the whole import instead of
being collected as a row error.

A good test suite for Task 2 checks that a bad row lands in
`result.errors` (no exception) and that valid rows in the same file
still import, so it should FAIL against this mutant, which lets the
ValueError from `_parse_row` propagate out of `import_catalog`.
"""
...
def import_catalog(source: str | Path | StringIO) -> ImportResult:
    ...
    for line_number, row in enumerate(reader, start=2):
        # MUTATION: no try/except around _parse_row -- a single bad row
        # now raises out of import_catalog instead of being recorded.
