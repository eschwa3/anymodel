...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("bulk-complete")
    p.add_argument("--job-id", type=int, action="append", required=True, dest="job_ids")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "create-customer":
            ...
            n = scheduler.complete_many_jobs(conn, args.job_ids)
            print(f"completed {n} jobs")
