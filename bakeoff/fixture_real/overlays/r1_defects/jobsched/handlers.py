...
def handle_search_jobs(conn: sqlite3.Connection, payload: dict, cfg: AppConfig | None = None) -> dict:
    """Backs the ops dashboard's free-text job search box."""
    cfg = cfg or load_config()
    query = str(payload.get("query", "")).strip()
    if not query:
        return {"ok": False, "error": "query is required"}
    matches = JobRepository(conn).search_by_name(query)
    page = matches[: cfg.search_page_size]
    return {
        "ok": True,
        "count": len(page),
        "total_matches": len(matches),
        "jobs": [{"job_id": j.id, "name": j.name, "status": j.status.value} for j in page],
    }
