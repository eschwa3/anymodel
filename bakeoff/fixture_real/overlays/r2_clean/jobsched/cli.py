...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("customer-summary")
    p.add_argument("--customer-id", type=int, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "create-customer":
            ...
            job = service.create_job(conn, args.customer_id, args.name, args.priority)
            print(f"created job {job.id} ({job.status.value})")
        elif args.command == "tick":
            job = scheduler.reserve_next_job(conn, args.worker_id, cfg)
            print(f"reserved job {job.id}" if job else "no pending jobs")
        elif args.command == "customer-summary":
            summary = service.get_customer_summary(conn, args.customer_id)
            print(
                f"{summary['name']} ({summary['plan_name']}): "
                f"{summary['active_jobs']}/{summary['job_quota'] or 'unlimited'} active, "
                f"{summary['total_jobs']} total"
            )
