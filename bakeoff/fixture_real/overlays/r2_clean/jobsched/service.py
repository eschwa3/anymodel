...
def get_customer_summary(conn: sqlite3.Connection, customer_id: int) -> dict:
    """A read-only rollup for the ops dashboard's customer detail page:
    which plan the customer is on, their quota, and how many of their jobs
    are currently active vs. the lifetime total.
    """
    customer = CustomerRepository(conn).get(customer_id)
    plan = PlanRepository(conn).get(customer.plan_id)
    jobs = JobRepository(conn).list_for_customer(customer.id)
    active = [j for j in jobs if j.status in _ACTIVE_JOB_STATUSES]
    return {
        "customer_id": customer.id,
        "name": customer.name,
        "plan_name": plan.name,
        "job_quota": plan.job_quota,
